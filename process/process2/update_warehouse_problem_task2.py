...
def update_warehouse_problem_task2(table_name, temporary_table_name, wh_query, wh_combined_table, delete_staging,
	print_internal=False, print_details=False):

	run = False
	if run == True:
		run_update_warehouse_problem_task2(table_name, temporary_table_name, wh_query, wh_combined_table, delete_staging,
		print_internal, print_details)
	else:
		u_print("WARNING: Warehousing turned off")

def run_update_warehouse_problem_task2(table_name, temporary_table_name, wh_query, wh_combined_table, delete_staging,
	print_internal=False, print_details=False):
	...
	u_print("Running Problem Task v2 Update Method")
	u_print("")
	...
	query_database2('Drop Problem Task Combined Table',drop_sql, output_db, output_database, print_details=print_details, ignore_errors=True)

	sql = get_sql_query('_CREATE_temp_problem_task_combined', warehousing_path+"/sql/"+project+"/")
	sql = sql.lower() #MAKE THE SQL LOWER CASE IN CASE ANY UPPER CASES HAVE SNUCK THROUGH
	sql = sql.replace('temp_problem_task_combined', wh_combined_table) #REPLACE THE COMBINED TABLE NAME WITH THE TEMP WH COMBINED NAME	
	...
	sql = get_sql_query("_MAIN_problem_task_detail_UPDATE", warehousing_path+"/sql/"+project+"/")	
	sql = sql.lower()
	sql = sql.replace('temp_problem_task_combined', wh_combined_table) #REPLACE THE COMBINED TABLE NAME WITH THE TEMP WH COMBINED NAME			
	query_database2('UPDATE RECORDS',sql, output_db, output_database, print_details=print_details)

	sql = get_sql_query("_MAIN_problem_task_detail_INSERT", warehousing_path+"/sql/"+project+"/")	
	sql = sql.lower()
	sql = sql.replace('temp_problem_task_combined', wh_combined_table) #REPLACE THE COMBINED TABLE NAME WITH THE TEMP WH COMBINED NAME			
