...
def setup_warehouse_problem2(output_database):
	u_print('########################################')
	...
	u_print("Running Problem v2 Setup Method")
	u_print("")
	...
	drop_sql = "DROP TABLE TEMP_problem_combined"
	query_database2('Drop Problem Combined Table',drop_sql, output_db, output_database, 
		print_details=print_details, ignore_errors=True)

	sql = get_sql_query('_CREATE_temp_problem_combined', "sql/"+project+"/")
	...
	dimension_table_list = [
		['company',"ISNULL(company,'')", 'TEMP_problem_combined',"company"],
	]
	...
	sql = get_sql_query('_CREATE_problem_detail', "sql/"+project+"/")
	query_database2('Creating Main Problem Detail Table', sql, 
		output_db, output_database, print_details=print_details)	
	
	#POPULATE THE TABLE
	sql = get_sql_query('_MAIN_problem_detail', "sql/"+project+"/")
	query_database2('Producing Main Problem Detail Table', sql, 
		output_db, output_database, print_details=print_details)	
