...
SELECT 
    ...
    inc.company,
    CASE 
    WHEN users.businessunit IS NULL THEN NULL
    ELSE users.businessunit 
    END AS businessunit, --MODDED
    ...
    parentincident_id
FROM
    dbo.MHCLG_incident inc

    LEFT JOIN dbo.MHCLG_sys_user users ON (users.sys_id = inc.customer_id)